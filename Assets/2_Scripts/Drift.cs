...
{
    [Header("전진/후진 가속도")] public float acceleration = 55f;
    [Header("조향 속도")] public float steering = 5f;
    [Header("낮을수록 더 미끄러짐")] public float driftFactor = 0.85f;
    [Header("최대 속도 제한")] public float maxSpeed = 300f;
    ...
    void FixedUpdate()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                float turnAmount = Input.GetAxis("Horizontal") * steering * Mathf.Clamp(speed / maxSpeed, 0.6f, 0.8f);

                if (isDriftMode)
                {
                    turnAmount *= 1.4f; // 드리프트 모드 시 조향 강화
                }
